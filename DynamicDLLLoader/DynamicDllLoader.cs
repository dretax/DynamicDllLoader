...
namespace DynamicDLLLoader
{
    public class DynamicDllLoader
    {
        ...
        {
            public uint
                CharacteristicsOrOriginalFirstThunk; // 0 for terminating null import descriptor; RVA to original unbound IAT (PIMAGE_THUNK_DATA)

            public uint
                TimeDateStamp; // 0 if not bound, -1 if bound, and real date\time stamp in IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT (new BIND); O.W. date/time stamp of DLL bound to (Old BIND)
                ...
                while (importDesc.Name > 0)
                {
                    ...
                    unsafe
                    {
                        ...
                        for (; *thunkRef > 0; thunkRef++, funcRef++)
                        {
                            ...
                            {
                                *funcRef = (uint) Win32Imports.GetProcAddress(new IntPtr(handle),
                                    new IntPtr(*thunkRef & 0xffff));
                            }
                            else
                            {
                        ...
                        importDesc = PointerHelpers.ToStruct<IMAGE_IMPORT_DESCRIPTOR>(codeBase,
                            ...
                            (uint) (Marshal.SizeOf(typeof(IMAGE_IMPORT_DESCRIPTOR)) * pcount));
                    }
                }
            }
        ...
        public void FinalizeSections(IntPtr headers, IMAGE_DOS_HEADER dosHeader, IMAGE_NT_HEADERS oldHeaders)
        {
            ...
            for (int i = 0; i < module.headers.FileHeader.NumberOfSections; i++)
            {
                ...
                {
                    bool aa = Win32Imports.VirtualFree(new IntPtr(section.PhysicalAddress),
                        (UIntPtr) section.SizeOfRawData, 0x4000);
                    continue;
                }
                ...
                {
                    uint oldProtect;
                    if (!Win32Imports.VirtualProtect(new IntPtr(section.PhysicalAddress), section.SizeOfRawData,
                        protect, out oldProtect))
                    {
                    }
                }
